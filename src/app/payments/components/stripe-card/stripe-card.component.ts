...
export class StripeCardComponent implements OnInit {
  ...
  @Input('monto') public monto : number = 0;
  public STRIPE: any;
  ...
  constructor( private formBuilder: FormBuilder, private payCardService: PaycardService) {
    this.donatorId = 1;
    this.projectId = 1;
  }

  async ngOnInit() {
    console.log(this.monto, "aqui")
    this.customForm.patchValue({amount: this.monto});
...
async onpay(): Promise<any> {
  if(this.customForm.invalid){
    ...
    try {
      ...
      //TODO: tok_2646
      this.payCardService.generateOrder({name: "Isaias", amount: this.monto, donatorId: this.donatorId, projectId: this.projectId})
      .subscribe( res => {
        console.log(res);
